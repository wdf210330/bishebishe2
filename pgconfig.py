...
def find_weather_presets():
    '''把天气名字换成我们能够读懂的文字，其中天气包含的内容
    Clear Night   Clear Noon   Clear Sunset
    Cloudy Night  Cloudy Noon  Cloudy Sunset
    Default       Dust Storm   Hard Rain Night            、
    Hard Rain Noon        Hard Rain Sunset         Mid Rain Sunset
    Mid Rainy Night       Mid Rainy Noon           Soft Rain Night
    Soft Rain Noon        Soft Rain Sunset         Wet Cloudy Night
    Wet Cloudy Noon       Wet Cloudy Sunset        Wet Night
    Wet Noon      Wet Sunset
    '''
...
def get_actor_blueprints(world, filter, generation):
    '''
    根据fliter这个关键词选出world中有关这个filter的所有蓝图
    然后因为carla中有很多种车辆，所以需要根据generation来选出我们想要的车辆
    '''
...
class World(object):
    ...
    def restart(self):
        ...
        if self.sync:     # 是否开启同步模式（仿真时间由客户端控制）
            self.world.tick()
        else:
...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                ...
                elif event.key == K_c and pygame.key.get_mods() & KMOD_SHIFT:  #这种是进行反向的切换
                    ...
                    try:
                        ...
                        pass
                elif event.key == K_t:  # 获得车辆实时的遥测数据
    ...
    def _parse_vehicle_keys(self, keys):
        from env import Env
        # self._control.throttle = Env.get_cmd()[0]
        # self._control.brake = Env.get_cmd()[2]
        # self._control.steer = Env.get_cmd()[1]
        ...
        if self._show_info:
            ...
            for item in self._info_text:
                if v_offset + 18 > self.dim[1]:
                    break
                if isinstance(item, list):  # 绘制如油门、刹车历史曲线
                    if len(item) > 1:
...
def game_loop(args=None):
    if args is None:
        ...
        argparser.add_argument('--width', type=int, default=1280)
        argparser.add_argument('--height', type=int, default=720)
        ...
        argparser.add_argument('--verbose', action='store_true', dest='debug')
